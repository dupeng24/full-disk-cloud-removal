...
class Decloud(nn.Module):
    def __init__(self, imagenet_model):
        ...
        self.attention3 = DehazeBlock(default_conv, 128, 3)
...
class cpa(nn.Module):
    def __init__(self, imagenet_model, model):
        ...
        self.tail1 = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(32, 1, kernel_size=7, padding=0), nn.Tanh())

    def forward(self, input):
        feature = self.feature_extract(input)
        decloud = self.tail1(feature)
        return decloud
