    ...
    with h5py.File(train_data, 'w') as h5f:
        ...
        for i in range(len(files2_clear)):
            ...
            for sca in scales:
                ...
                hazy = np.expand_dims(hazy, axis=0)
                clear = np.expand_dims(clear, axis=0)
                hazy = img_to_patches(hazy, size, stride)
                clear = img_to_patches(clear, size, stride)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--patch_size", "--p", type=int, default=1024, help="Patch size")
    parser.add_argument("--stride", "--s", type=int, default=600, help="Size of stride")
